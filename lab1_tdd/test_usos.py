...
class InputTesting(unittest.TestCase):
    ...
    def test_input_value_and_clear_button(self):
        ...
        login_box.send_keys("testowy_login")
        inputValue = login_box.get_attribute("value")
        self.assertEqual("testowy_login", inputValue)

        try:
            clear_button = self.driver.find_element(By.XPATH, "//*[@type='reset' or @name='clear' or contains(text(), 'Wyczyść')]")
            clear_button.click()
        except Exception:
            self.fail("Przycisk 'Wyczyść' nie został znaleziony na stronie!")

        clearedValue = login_box.get_attribute("value")
        self.assertEqual("", clearedValue, "Błąd: Pole logowania nie zostało wyczyszczone po kliknięciu!")
